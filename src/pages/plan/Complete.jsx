...
const Complete = () => {
  ...
  const settings = {
    infinite: false,
    speed: 500,
    slidesToShow: daysCount > 0 ? (daysCount > 3 ? 4 : daysCount) : 1,
    slidesToScroll: 1,
    swipe: true,
    responsive: [
      {
        breakpoint: 1200,
        settings: {
          slidesToShow: daysCount > 0 ? (daysCount > 3 ? 2 : daysCount) : 1,
          slidesToScroll: 1,
        },
      },
    ],
  ...
  const handleSaveToMyPage = async () => {
    try {
      ...
      if (payload.planId === null) {
        response = await axios.post('http://localhost/plans', payload);
      } else {
        response = await axios.put(
          `http://localhost/plans/${payload.planId}`,
          payload
        );
      }
          ...
          <DaySlide key={dayIndex}>
            ...
            <ol>
              {dayPlaces.map((place, index) => {
                const route =
                  plan.routes?.find(r => r.endTime === place.startTime) ?? null;
                  ...
                  <li key={place.placeId} className="flex">
                    ...
                    <div className="place_info">
                      <span className="place_time pt_blue size_xxs weight_sb">
                        {new Date(place.startTime).toLocaleTimeString([], {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </span>
                      <strong className="place_name size_sm weight_sb">
                        {place.placeName}
                      </strong>
                      <span className="place_category size_xxs pt_gy">
                        {place.category}
                      </span>
                    ...
                    <div className="place_price">
                      ...
                      <span className="pt_pink size_sm weight_sb">
