...
const ProduceModify = () => {
  ...
  useEffect(() => {
    const fetchPlanData = async () => {
      try {
        ...
        fetchedPlan.places.forEach((place, i) => {
  ...
  const findRoute = nextPlace => {
    return plan.routes.find(route => route.endTime === nextPlace.startTime);
  };
  ...
  // 총 금액 계산 (장소 가격 + 교통비)
  const totalPlacePrice = places.reduce(
    (acc, place) => acc + (place.price || 0),
    0
  );
  const totalRoutePrice = plan.routes.reduce(
    (acc, route) => acc + (route.price || 0),
    0
  );
  ...
  // 교통비 합산
  const totalTransportCost = plan.routes.reduce(
    (acc, route) => acc + (route.price || 0),
    0
  );
  ...
  const fetchCoordinates = async address => {
    try {
      const response = await axios.get(
        'https://openapi.naver.com/v1/map/geocode.json',
        {
