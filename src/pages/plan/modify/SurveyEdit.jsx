...
const SurveyEdit = () => {
  ...
  const handleFinish = async () => {
    try {
      ...
      // 세션 스토리지에 저장
      data.places.sort((a, b) => new Date(a.startTime) - new Date(b.startTime));
      data.routes.sort((a, b) => new Date(a.startTime) - new Date(b.startTime));
...
const editPlanDays = (plan, newStartDate) => {
  ...
  const dateDifferenceInDays = Math.floor(
    (newStartDate - originalStartDate) / (1000 * 60 * 60 * 24)
  );

  const updatedPlaces = plan.places.map(place => {
    const updatedStartTime = new Date(place.startTime);
    updatedStartTime.setDate(
      updatedStartTime.getDate() + dateDifferenceInDays + 1
    );
